...
// AddProgram 添加要监控的程序
func (pm *ProgramManager) AddProgram(name, path string, args []string, maxRestarts int) {
	program := &ProgramInfo{
		Name:        name,
		Path:        path,
		Args:        args,
...
func (pm *ProgramManager) StartProgram(program *ProgramInfo) error {
	...
	if !program.Enabled {
		return fmt.Errorf("程序 %s 已被禁用", program.Name)
	}

	// 检查程序文件是否存在
	fileInfo, err := os.Stat(program.Path)
	if os.IsNotExist(err) {
		return fmt.Errorf("程序文件不存在: %s", program.Path)
	}
	if err != nil {
	...
	// 判断路径是否是目录
	if fileInfo.IsDir() {
		return fmt.Errorf("路径是目录而不是文件: %s", program.Path)
	}
	...
	if currentMode&0111 == 0 {
		...
		err := os.Chmod(program.Path, newMode)
		if err != nil {
	...
	// 创建命令
	program.Cmd = exec.Command(program.Path, program.Args...)
	...
	if err := program.Cmd.Start(); err != nil {
		return fmt.Errorf("启动程序 %s 失败: %v", program.Name, err)
	}

	program.Pid = program.Cmd.Process.Pid
	log.Printf("程序 %s 已启动 (PID: %d)", program.Name, program.Pid)
		...
		for {
			...
			case sig := <-pm.sigCh:
				...
				case syscall.SIGINT, syscall.SIGTERM, syscall.SIGQUIT:
					log.Printf("收到信号 %v，正在停止所有程序...", sig)
					pm.StopAll()
					close(pm.stopCh)
					os.Exit(0)
...
func main() {
	...
	log.Println("多程序监控管理器启动 (SIGCHLD自动重启)")

	// 创建程序管理器
	manager := NewProgramManager()

	// 参数：程序名称, 程序路径, 参数列表, 最大重启次数(-1表示无限重启)
	manager.AddProgram("消息转发服务1", "/root/gostudy/MessageForwording/MessageForward", []string{"-listenport", "9000", "-targetaddr", "192.168.104.100:10000"}, -1)
	manager.AddProgram("消息转发服务2", "/root/gostudy/MessageForwording/MessageForward", []string{"-listenport", "9001", "-targetaddr", "192.168.104.100:10000"}, -1)
	manager.AddProgram("消息转发服务3", "/root/gostudy/MessageForwording/MessageForward", []string{"-listenport", "9002", "-targetaddr", "192.168.104.100:10000"}, -1)
